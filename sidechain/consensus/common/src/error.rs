...
pub enum Error {
	...
	NodeMetadata(itp_node_api_metadata::Error),
	#[error("Node metadata provider error: {0:?}")]
	NodeMetadataProvider(#[from] itp_node_api_metadata_provider::Error),
}
...
impl From<itp_node_api_metadata::Error> for Error {
	fn from(e: itp_node_api_metadata::Error) -> Self {
		Self::NodeMetadata(e)
	}
}
